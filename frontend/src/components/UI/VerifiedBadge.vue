...
<template>
  <div>
    <div class="relative verified cursor-pointer" ref="verifiedDefined">
      <svg xmlns="http://www.w3.org/2000/svg" width="16.12" height="16" viewBox="0 0 16.12 16">
        <path id="Path_645" data-name="Path 645"
          ...
          transform="translate(-384.801 -186.092)" fill="#0ab5e7" />
      </svg>
      <div :style="{right: leftPopup ? '100%' : 'auto'}" class="rounded-lg py-2 px-2 verified--defined font-normal text-sm">
        A verified badge for confirms that this is an authentic Page for this public figure, media
        company or brand
      </div>
    </div>
  </div>
...
const verifiedDefined = ref(null);
const leftPopup = ref(false)

onMounted(() => {
  const popup = verifiedDefined.value;
  if (popup) {
    const screenWidth = screen.width / 3 * 2;
    const rect = popup.getBoundingClientRect();
    const x = rect.left + window.scrollX;
    if (x > screenWidth) {
      leftPopup.value = true
    }
    else leftPopup.value = false
  }
});
</script>